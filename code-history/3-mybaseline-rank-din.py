...
#trn_user_item_feats_df,tst_user_item_feats_df=trn_user_item_feats_df[:10000],tst_user_item_feats_df[:10000]
...
# 在原有的特征上拼上 历史行为list
trn_user_item_feats_df_din_model = trn_user_item_feats_df.merge(his_behavior_df, on='user_id')

if offline:
    val_user_item_feats_df_din_model = val_user_item_feats_df.merge(his_behavior_df, on='user_id')
else:
    val_user_item_feats_df_din_model = None
    val_user_item_feats_df_din_model = val_user_item_feats_df.merge(his_behavior_df, on='user_id')

tst_user_item_feats_df_din_model = tst_user_item_feats_df.merge(his_behavior_df, on='user_id')

all_df=trn_user_item_feats_df_din_model.append(val_user_item_feats_df_din_model)
all_df=all_df.append(tst_user_item_feats_df_din_model)
#all_df=trn_user_item_feats_df_din_model.append(tst_user_item_feats_df_din_model)
...
# 数据准备函数
def get_din_feats_columns(alldf,df, dense_fea, sparse_fea, behavior_fea, his_behavior_fea, emb_dim=32, max_len=50):
    """
    数据准备函数:
    df: 数据集
    dense_fea: 数值型特征列
    ...
    for feat in sparse_fea:
        print(feat)
        print(alldf[feat].max())
        sparse_feature_columns.append(SparseFeat(feat, vocabulary_size=df[feat].max() + 1, embedding_dim=emb_dim))
    #sparse_feature_columns = [SparseFeat(feat, vocabulary_size=alldf[feat].nunique() + 1, embedding_dim=emb_dim) for feat in sparse_fea]
...
# 把特征分开
sparse_fea = ['user_id', 'click_article_id', 'category_id']

behavior_fea = ['click_article_id']

hist_behavior_fea = ['hist_click_article_id']

dense_fea = ['time_diff0', 'word_diff0', 'score','rank','words_count']
#dense_fea = ['word_diff0', 'score','rank','words_count']
...
for feat in dense_fea:
    trn_user_item_feats_df_din_model[feat] = mm.fit_transform(trn_user_item_feats_df_din_model[[feat]].values)
...
# 准备训练数据
trn_user_item_feats_df_din_model=trn_user_item_feats_df_din_model.append(val_user_item_feats_df_din_model)
x_trn, dnn_feature_columns = get_din_feats_columns(all_df,trn_user_item_feats_df_din_model, dense_fea, 
                                               sparse_fea, behavior_fea, hist_behavior_fea, max_len=50)
y_trn = trn_user_item_feats_df_din_model['label'].values

if offline:
    # 准备验证数据
    x_val, dnn_feature_columns = get_din_feats_columns(all_df,val_user_item_feats_df_din_model, dense_fea, 
                                                   sparse_fea, behavior_fea, hist_behavior_fea, max_len=50)
    y_val = val_user_item_feats_df_din_model['label'].values
    
dense_fea = [x for x in dense_fea if x != 'label']
x_tst, dnn_feature_columns = get_din_feats_columns(all_df,tst_user_item_feats_df_din_model, dense_fea, 
                                               sparse_fea, behavior_fea, hist_behavior_fea, max_len=50)
...
if offline:
    ...
    #val_user_item_feats_df_din_model['pred_score'] = model.predict(x_tst, verbose=1, batch_size=256)
    #val_user_item_feats_df_din_model[['user_id', 'click_article_id', 'pred_score']].to_csv(save_path + 'din_rank_score_val.csv', index=False)
    #rank_results = val_user_item_feats_df_din_model[['user_id', 'click_article_id', 'pred_score']]
